...
See [tutorial](https://satijalab.org/seurat/articles/pbmc3k_tutorial.html).
...
### 3a. QC and selecting cells for further analysis
...
You can display the object metadata to see QC metrics:
...
### 3c. Feature selection: identification of highly variable features
...
# improve speed by only scaling genes that will be used in PCA (previously identified as top 2000 highly variable genes) using `ScaleData(pbmc)` with no `features` argument. If you do this, PCA and clustering results will be unaffected but heatmaps will not work properly
...
### 4b. Determine dimensionality of dataset
...
```{r jackstraw plot, warning=FALSE}
...
## 6. Non-linear dimensional reduction (UMAP/tSNE)
...
```{r roc differential expression test}

cluster0.markers <- FindMarkers(pbmc, ident.1 = 0, logfc.threshold = 0.25, test.use = "roc", only.pos = TRUE)

```

### 7b. Visualize marker expression

We include several tools for visualizing marker expression. `VlnPlot()` (shows expression probability distributions across clusters), and `FeaturePlot()` (visualizes feature expression on a tSNE or PCA plot) are our most commonly used visualizations. We also suggest exploring `RidgePlot()`, `CellScatter()`, and `DotPlot()` as additional methods to view your dataset. See the [visualization vignette](https://satijalab.org/seurat/articles/visualization_vignette.html) for more details.


```{r differential expression visualization}
...
```{r differential expression visualization 2, fig.height=15, fig.width = 15}
...
table <- data.frame("Cluster ID" = c(0,1,2,3,4,5,6,7,8),
             "Markers" = c("IL7R, CCR7", "CD14, LYZ", "IL7R, S100A4", "MS4A1", "CD8A", "FCGR3A, MS4A7", "GNLY, NKG7", "FCER1A, CST3", "PPBP"),
             "Cell Type" = c("Naive CD4+ T", "CD14+ Mono", "Memory CD4+", "B", "CD8+ T", "FCGR3A+ Mono", "NK", "DC", "Platelet"))
...
## 9. Data visualization extras

Seurat-generated plots are `ggplot2`-compatible and can be edited using standard `ggplot2` commands.

```{r ggplot2 plot editing}
baseplot <- DimPlot(pbmc, reduction = "umap")
baseplot + labs(title = "Clustering of 2,700 PBMCs") 
baseplot + labs(title = "Clustering of 2,700 PBMCs") + theme_dark()
```

### 9a. Interactive plotting features
...
HoverLocator(plot = baseplot, information = FetchData(pbmc, vars = c("ident", "PC_1", "nFeature_RNA")))

```


There are several plots available in Seurat to visualize your data.

```{r extra plots, fig.width=15, fig.height=15}
...
```{r extra plots 2, fig.width=15, fig.height=5}
